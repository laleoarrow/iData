...
When macOS routes a supported file to `iData` because `iData` is the default handler for that format, `iData` should only take ownership of the file if the file is larger than 100 MB. Smaller files should be silently forwarded to a non-`iData` application such as Excel, WPS, or TextEdit.
...
1. If `iData` receives a file-open event for a regular file with a supported table-like suffix and the file size is greater than `100 * 1024 * 1024` bytes, `iData` should open the file normally with `VisiData`.
2. If the same kind of file is `<= 100 * 1024 * 1024` bytes, `iData` should silently forward the file to a non-`iData` application instead of opening an `iData` session.
...
- open in `iData` only when size is strictly greater than 100 MiB
- forward when size is less than or equal to 100 MiB
...
1. supported file larger than 100 MiB routes into `iData`
2. supported file exactly 100 MiB forwards away from `iData`
3. supported file smaller than 100 MiB forwards away from `iData`
...
- `100 MB` here is implemented as `100 MiB` (`100 * 1024 * 1024` bytes) for deterministic behavior in code.
